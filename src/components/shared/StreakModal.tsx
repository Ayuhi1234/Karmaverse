...
export function StreakModal({ visible, onClose, streak }: { visible: boolean; onClose: () => void; streak: number; }) {
  ...
  return (
    <StatModal visible={visible} onClose={onClose} title="Your streak">
      <View style={s.hero}>
        <View style={s.flameCircle}><Flame size={30} color="#fb923c" fill="#fdba74" /></View>
        <View style={{ flex: 1 }}>
      ...
      <Text style={s.tip}>Do one green action a day — schedule a pickup or play the eco quiz — to keep your streak alive.</Text>
    </StatModal>
  );
}
...
const s = StyleSheet.create({
  ...
  flameCircle: { width: 56, height: 56, borderRadius: 28, backgroundColor: '#fff7ed', alignItems: 'center', justifyContent: 'center' },
  streakNum: { fontSize: 24, fontWeight: '900', color: '#0f172a' },
  streakSub: { fontSize: 13, fontWeight: '600', color: '#64748b', marginTop: 2 },
  ...
  dotIdle: { backgroundColor: '#f1f5f9' },
  ...
  track: { height: 8, borderRadius: 4, backgroundColor: '#f1f5f9', overflow: 'hidden' },
